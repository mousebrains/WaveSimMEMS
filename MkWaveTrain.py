...
def mkTrain(data:dict, wave:dict) -> dict:
    depth = data["depth"] # Water depth
    duration = data["duration"] # How long a time period we need to cover
    tMin = duration + wave["period"] # extra period for an initial random phase
    minPeriod = wave["period"] / 2 # No shorter than 1/2 a desired period
    nWaves = math.ceil(duration / wave["period"]) # Inital guess at the number we need to generate
    for cnt in range(5, 50, 5): # Increase number generated by cnt
        period = np.random.normal(wave["period"], wave["periodSigma"], size=(nWaves+cnt,))
